...
impl GlobalState {
    pub fn terminal_tab_view(&self) -> Element<'static, GlobalEvents> {
        let t =
            row![
                button(text("New Terminal").center().size(16)).on_press(GlobalEvents::UiEvents(
                    UiMessages::TerminalEvents(TerminalEvents::NewTerminal)
                ))
            ]
            .extend(self.ui_state.terminals.iter().map(|t| {
                ...
                row![
                    button(text(format!("{}", term_name)).center().size(16))
                        ...
                        .on_press(GlobalEvents::UiEvents(UiMessages::TerminalEvents(
